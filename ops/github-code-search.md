...
A completed selected scope can be run again without making a request. Deferred work resumes only for a recorded provider or budget condition, after a previously missing explicit token is supplied, or for an explicit `--resume-query-budget`; unrelated deferred work is never promoted automatically. A plan with an actual missing or invalid repository scope reports that gap without running the fixture control.
...
- A successful page is checkpointed. Rerunning `run-code` resumes budget-deferred work from its next page. `--resume-query-budget N` promotes up to N additionally deferred queries. `--retry-failed` explicitly retries only `AUTH_FAILED` (after the operator replaces the selected explicit token), `REQUEST_FAILED` (the runner does not determine whether its cause is transient), index-change, and overlapping-page failures. Index-change or overlapping-page failures restart that query and retain earlier observations. It never promotes scope, publicness, metadata, incomplete-coverage, or provider-cap failures.
