...
`complete` in a browser result means that its bounded capture and automated analysis finished. `content_review_complete` remains false until a separate publication-intent/sensitivity review supplies evidence. Neither a completed parser nor a clean rule result proves absence of sensitive content. A truncated identity-encoded probe response may retain provisional signals from its captured prefix; its `analysis_complete` and `capture_complete` remain false. No additional content is fetched for this prefix review.
