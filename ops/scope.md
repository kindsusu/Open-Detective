...
The required sequence is administrator export of contracts, assets, and processor/outsourcer registers → scope approval → candidate import. Authorized owner inventory credentials are a separate provider/`--scope-id` path under `SUD-R09`; they do not flow from the intake and do not authorize target measurement.
