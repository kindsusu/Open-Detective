...
2. Build identifiers and `search-plan` offline from the intake. This does not authorize a deployment request.
3. Run `doctor --reference .` to identify the runtime root actually loaded by the command and compare it with the reviewed source tree at `.`.
4. Run fresh controls under their own executable control scope with `channels-doctor`; only then run `github-discover` or `search-plan` under their separate `scope_id` and channel-health contract. A successful control does not establish company discovery or approve a deployment target.
5. Review public-metadata candidates and obtain administrator exports for contracts, assets, and processor/outsourcer registers. For candidates from those exports, approve executable target scope before importing them. Owner-inventory credentials remain a separate authorization path.
6. Confirm ownership and approve the exact target scope before binding its locator to an asset and sending a target request. A public deployment can outlive a private repository.
...
9. Confirm content only from minimal evidence. Stop when personal data or a usable secret is visible. Live secret validity remains unknown unless an authorized owner-side check establishes it. `asset-profile` is a separate offline analysis of owner-authorized local bytes; a sanitized probe report does not supply those bytes.
10. For active exposure, the accountable owner may isolate the exact deployment and preserve logs based on harm before final classification. Plan continuity, secret rotation, and owner-side validity checks separately. Keep application authorization checks behind SSO.
