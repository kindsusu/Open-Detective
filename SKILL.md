...
For file-content discovery, enable optional `github_code` jobs with `search-plan plan --github-code` or `search-plan enable-code --plan ...`. Use `search-plan run-code` separately with an explicit search-token environment name, a fresh public-code fixture control, and private locator storage. Reuse existing query jobs; do not duplicate account discovery or treat code hits as deployed assets. Code-search authentication is restricted to the GitHub search API and never enters anonymous measurements. The generic importer cannot complete code jobs. Inspect their rate/pagination/index gaps and `ownership_pending` file candidates before following the existing ownership and target-scope workflow. Read `ops/github-code-search.md` for commands and control setup.
